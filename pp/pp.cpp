...
void	clearScreen(const game* lobby)
{
...
{
	game*		window	= new game(640, 480, "PP");
	Player*		playerL	= new Player(window->render, { 10, 10, 20, 80 } );
	Player*		playerR	= new Player(window->render, { 630, 10, 20, 80 });
	Ball*		ball	= new Ball(window->render, { 320, 240, 10, 10 }, playerL, playerR);
	...
	while (running) {
		...
		while (SDL_PollEvent(&window->event)) {
			...
			//  if (lobby->event.key.keysym.sym == SDLK_a) { playerL->move('L'); }
				if (window->event.key.keysym.sym == SDLK_s) { playerL->move('D'); }
			//  if (lobby->event.key.keysym.sym == SDLK_d) { playerL->move('R'); }

				if (window->event.key.keysym.sym == SDLK_UP		) { playerR->move('U'); }
			//	if (lobby->event.key.keysym.sym == SDLK_LEFT	) { playerR->move('L'); }
				if (window->event.key.keysym.sym == SDLK_DOWN	) { playerR->move('D'); }
			//	if (lobby->event.key.keysym.sym == SDLK_RIGHT	) { playerR->move('R'); }
				break;
			case SDL_QUIT:
				running = false;
			}
